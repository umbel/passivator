...
def user_to_profile(user):
    ...
    if location:
        ...
        location = find_location(user)
        profile['address'] = {
            'city': location.split(',')[3],
            'state': location.split(',')[5]
        }
...
def find_location(user):
    geo = user.get('geo')
    if geo is not None:
        coordinates = geo.get('coordinates')[0]
    else:
        coordinates = json.loads(data).get('place').get('bounding_box').get('coordinates')[0][0]
    location = geolocator.reverse("%s,%s" % (coordinates[1], coordinates[0])).address
    return location
